...
config_parent_path = os.path.abspath(os.path.dirname(__file__))
config_path = os.path.join(config_parent_path, 'i3')
make_link(config_path, target)
make_link('%s/Xmodmap' % config_parent_path, '%s/.Xmodmap' % get_home_dir())
...
os.system("sudo apt-get -y install xbacklight")
os.system("sudo apt-get -y install feh")
