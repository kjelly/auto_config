...
def main [ ms:duration = 1300ms ] {
  ...
  if (has_new_process | not $in) {
    ...
    let windows = (tmux list-windows -F "#{pane_current_path}***#{window_id}***#{pane_title}***-#{window_activity}***-#{window_active}"|lines|each {|it| $it|split column '***' path id title active_time active}|flatten|each {|it| $it |upsert active_time ($it.active_time|into int) | upsert active ($it.active | into int) }|sort-by --natural path title)
