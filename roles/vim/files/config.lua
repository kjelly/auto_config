    ...
    if type(a) == 'table' then
        if type(b) == "table" then
            if vim.tbl_count(a) > vim.tbl_count(b) then
                return setmetatable(a, {__newindex = function()
                    return b
                end})
            else
                return setmetatable(b, {__newindex = function()
                    return a
                end})
            end
        else
...
local langservers = {
    ...
    'gopls', 'graphql', 'html', 'jsonls', 'marksman', 'pyright',
    'rust_analyzer', 'sqlls', 'lua_ls', 'terraformls', 'tsserver', 'vimls',
    'yamlls', 'ruff_lsp'
}
...
SafeRequireCallback('lualine', function(lualine)
    ...
    lualine.setup {
        ...
        sections = {
            lualine_a = {'mode', tab_num},
            lualine_b = {
                {getModified, color = {fg = 'red'}}, 'diagnostics', 'branch',
                'diff'
            },
            lualine_c = {
                {floatermInfo, cond = HasTerminal}, {'filename', path = 1}
            },
            lualine_x = {
                {
    ...
    for _, lsp in pairs(langservers) do
        ...
        for _, v in pairs(vim.tbl_keys(LSP_CONFIG)) do
            if LSP_CONFIG[v][lsp] ~= nil then
                lspconfig_setup_opts[v] =
                    vim.tbl_extend("force", lspconfig_setup_opts[v] or {},
                                   LSP_CONFIG[v][lsp])
            end
        end
...
local function getWorkspaceVimPath(type)
    ...
    local workspaceConfigPath =
        workspace_path .. convertName(vim.fn.getcwd()) .. '.' .. type
        ...
        elseif mode == 'n' then
            Defer(function() vim.cmd("FloatermShow") end, function()
                ...
                vim.fn.feedkeys(vim.api.nvim_replace_termcodes("<c-p>", true,
                                                               true, true), 't')
                vim.fn.feedkeys(vim.api.nvim_replace_termcodes("<cr>", true,
                                                               true, true), 't')
            end)
        elseif mode == 'i' then
...
function RunShellAndShow(command)
    Defer(function()
        if HasTerminal() == false then vim.cmd("FloatermNew") end
    end, function() vim.cmd("FloatermShow") end,
...
function NextItem(offset)
    local function inner()
        ...
        elseif vim.api.nvim_eval("len(filter(getwininfo(), 'v:val.quickfix'))") >
            0 then
            if offset > 0 then
                vim.cmd("cn")
            else
...
SafeRequire('cokeline').setup({
    default_hl = {
        fg = function(buffer)
            return buffer.is_focused and '#ff0000' or '#116822'
        end,
        bg = '#cccccc'
    },
    sidebar = {
    ...
    local job = Job:new({
        ...
        on_exit = function(_, exitcode)
            vim.defer_fn(function()
                ...
                vim.fn.setbufline(bufID, result[1], string.format(
                                      '--- output --- [%d]', exitcode))
            end, 100)
        end
    })
...
function KillAndRerunTerm(name, command, opts)
    ...
    if opts.notify ~= "" or opts.notify ~= nil then
        opts.shell = true
        notify_command = string.format(";hterm-notify '%s' '%s'", opts.notify,
                                       name)
    end
    local autoclose = 0
    ...
    if opts.shell then
        ...
        vim.cmd(string.format('FloatermNew --autoclose=%d --name=%s %s',
                              autoclose, name, command, notify_command))
    end
end
