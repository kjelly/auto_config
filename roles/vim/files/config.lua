...
function SafeRequire(name)
    ...
    return setmetatable({},
                        {__index = function(t, key) return function() end end})
end
...
function DelaySetup2()
  ...
  vmap g<C-x> <Plug>(dial-decrement-additional)
    ...
    if Random(1, 100) < 10 then UpdatePlug() end
...
function DelaySetup1()
    ...
    SafeRequireCallback('lspsaga', function(saga) saga.init_lsp_saga() end)
...
function RunPreviousCommandFunc()
    ...
    if mode == 't' then
        vim.fn.feedkeys(vim.api
                        ...
                        't')
        vim.fn.feedkeys(
            vim.api.nvim_replace_termcodes("<cr>", true, true, true), 't')
    elseif mode == 'n' then
...
function KillAndRerunTerm(name, command)
    ...
    vim.cmd(string.format('FloatermNew --autoclose=0 --name=%s %s', name,
                          command))
end
