...
local lazyPackages = {
  ...
  { 'https://github.com/lukas-reineke/indent-blankline.nvim', main = "ibl" },
  ...
  { 'https://github.com/akinsho/git-conflict.nvim', version = "*",                                        config = true },
...
function FindFileCwd()
  ...
  Job:new({
    ...
    on_exit = function(_, exitcode)
      if exitcode == 0 then
        vim.defer_fn(function() SafeRequire('fzf-lua').git_files() end, 100)
      else
        vim.defer_fn(function() SafeRequire('fzf-lua').files() end, 100)
      end
    end,
  }):start()
end
...
local function updateEinkWidth()
  ...
  local job = Job:new({
    ...
    on_exit = function(_, _) vim.schedule(checkIsEink) end,
  })
  job:start()
end
